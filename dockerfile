...
RUN apk update && \
    apk --no-cache add bash git openssh-client openssl supervisor libzip-dev zip unzip ghostscript \
    postgresql-dev mysql-dev curl imagemagick imagemagick-dev autoconf g++ make freetype-dev \
    libjpeg-turbo-dev libpng-dev && \
    docker-php-ext-configure gd --with-freetype --with-jpeg && \
    docker-php-ext-install pdo_pgsql pdo_mysql bcmath zip gd && \
